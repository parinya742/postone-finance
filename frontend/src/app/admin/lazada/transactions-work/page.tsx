...
export default function LazadaTransactionsWorkPage() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      const filename = `lazada-transactions-work-${dateStr}.xlsx`
      XLSX.writeFile(wb, filename)

      api.post('/audit-logs', {
        action: 'export',
        target_type: 'lazada-transactions-work',
        target_id: 0,
  ...
  const { data, isLoading } = useQuery<PaginatedResponse<LazadaTransactionWork>>({
    queryKey: ['lazada-transactions-work', search, page, shopName, startDate, endDate, txType, paidStatus, source],
    queryFn: () =>
      api.get('/lazada/transactions-work', {
        params: {
          search: search || undefined,
          page,
          per_page: 50,
  ...
  const hasFilter = !!(search || shopName || startDate || endDate || txType || paidStatus || source)
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-slate-800">รายการธุรกรรม Lazada (Work)</h1>
          </div>
          ...
          <table className="w-full text-xs" style={{ minWidth: '2600px' }}>
            ...
            <tbody className="divide-y divide-slate-100">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={25} className="px-5 py-12 text-center text-slate-400">
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <TD center>
                      {item.source ? (
                        <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${item.source === 'openapi' ? 'bg-purple-100 text-purple-700' : 'bg-teal-100 text-teal-700'}`}>
                          {item.source}
                        </span>
                      ) : <span className="text-slate-300">—</span>}
                    </TD>
      ...
      <p className="text-xs text-slate-400 flex items-center gap-1">
        ...
        ข้อมูลจากตาราง lazada_transactions_work · แสดงผลตามลำดับคอลัมน์ของ Excel
      </p>
    </div>
  )
}
