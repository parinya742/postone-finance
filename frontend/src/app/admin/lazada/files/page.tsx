    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-slate-800">ไฟล์ส่งออก Lazada</h1>
        </div>
        ...
        <table className="w-full text-sm min-w-[900px]">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="text-right px-5 py-3 font-medium text-slate-600">ยอดรวม (฿)</th>
            ...
            {isLoading ? (
              ...
              items.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                  <td className='text-xs text-slate-500 max-w-[120px] truncate text-center'>{item.id}</td>
                  <td className="px-5 py-4">
                    <p className="font-medium text-slate-800 text-sm">{item.shop_name}</p>
                    {item.short_code && (
                      <span className="text-xs font-mono text-orange-600">{item.short_code}</span>
                    )}
                  </td>
                  <td className="px-5 py-4">
                    <p className="text-xs font-mono text-slate-700 truncate max-w-[240px]" title={item.file_name ?? ''}>
                      {item.file_name ?? '—'}
                    </p>
                  </td>
                  ...
                  <td className="px-5 py-4 text-right">
                    {item.total_amount != null ? (
                      <span className={`font-semibold ${item.total_amount < 0 ? 'text-red-600' : 'text-green-700'}`}>
