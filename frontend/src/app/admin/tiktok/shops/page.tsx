...
import { TikTokShop, AuditLog, PaginatedResponse } from '@/lib/types'
...
function ShopModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (item) {
        await api.put(`/tiktok/shops/${item.id}`, payload)
      } else {
        await api.post('/tiktok/shops', payload)
      }
      onSuccess()
    } catch (err: unknown) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 sticky top-0 bg-white z-10">
          <h2 className="font-semibold text-slate-800">
            {item ? 'แก้ไขร้านค้า TikTok' : 'เพิ่มร้านค้า TikTok'}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <input
                ...
                required
                maxLength={255}
                placeholder="เช่น My TikTok Shop"
                className="w-full border border-slate-300 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-rose-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                maxLength={255}
                placeholder="กรอกภายหลังได้"
                className="w-full border border-slate-300 rounded-lg px-3.5 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-rose-500"
              />
            </div>
            ...
            <div>
              ...
              <input
                value={form.shops_code}
                onChange={set('shops_code')}
                maxLength={255}
                placeholder="กรอกภายหลังได้"
                className="w-full border border-slate-300 rounded-lg px-3.5 py-2.5 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-rose-500"
              />
          ...
          <div className="border-t border-slate-100 pt-4">
            ...
            <div>
              ...
              <input
                value={appKey || 'ยังไม่ได้ตั้งค่า TIKTOK_APP_KEY'}
                disabled
                readOnly
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full border border-slate-300 rounded-lg px-3.5 py-2.5 pr-10 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-rose-500"
                    />
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={saving}
              className="flex-1 bg-rose-500 hover:bg-rose-600 disabled:opacity-60 text-white rounded-lg py-2.5 text-sm font-semibold transition-colors"
            >
...
export default function TikTokShopsPage() {
  ...
  const [editingItem, setEditingItem] = useState<TikTokShop | null>(null)
  ...
  const { data: authConfig } = useQuery<{ app_key: string }>({
    queryKey: ['tiktok-auth-config'],
    queryFn: () => api.get('/tiktok/auth-config').then((r) => r.data),
    enabled: can('tiktok-shops.view'),
  })

  const { data, isLoading } = useQuery<PaginatedResponse<TikTokShop>>({
    queryKey: ['tiktok-shops', search, page],
    queryFn: () =>
      api.get('/tiktok/shops', { params: { search, page, per_page: 50 } }).then((r) => r.data),
    enabled: can('tiktok-shops.view'),
  })

  const { data: auditData, isLoading: auditLoading } = useQuery<PaginatedResponse<AuditLog>>({
    queryKey: ['tiktok-shop-audit'],
    queryFn: () => api.get('/audit-logs', { params: { target_type: 'tiktok_shop', per_page: 30 } }).then((r) => r.data),
    enabled: can('tiktok-shops.view'),
  })

  const deleteItem = useMutation({
    mutationFn: (id: number) => api.delete(`/tiktok/shops/${id}`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['tiktok-shops'] })
      qc.invalidateQueries({ queryKey: ['tiktok-shop-audit'] })
    },
  })

  const refreshToken = useMutation({
    mutationFn: (id: number) => api.post(`/tiktok/shops/${id}/refresh-token`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['tiktok-shops'] })
      qc.invalidateQueries({ queryKey: ['tiktok-shop-audit'] })
    },
  ...
  const getTokenStatus = (item: TikTokShop) => {
  ...
  const handleGetAuthUrl = async (item: TikTokShop) => {
    setAuthUrlLoading(item.id)
    try {
      const res = await api.get(`/tiktok/shops/${item.id}/auth-url`)
      window.open(res.data.url, '_blank')
      setCodeInput('')
      setCodeError('')
      setCodeModal({ shopId: item.id, shopName: item.seller_name ?? `Shop #${item.id}` })
    } catch {
      alert('ไม่สามารถสร้าง URL ได้ กรุณาตรวจสอบ TIKTOK_APP_KEY ใน .env')
    } finally {
  ...
  const handleExchangeCode = async () => {
    ...
    setCodeError('')
    try {
      await api.post(`/tiktok/shops/${codeModal.shopId}/exchange-token`, { code: codeInput.trim() })
      setCodeModal(null)
      qc.invalidateQueries({ queryKey: ['tiktok-shops'] })
      qc.invalidateQueries({ queryKey: ['tiktok-shop-audit'] })
    } catch (err: unknown) {
      const tiktokErr = (err as { response?: { data?: { tiktok_error?: { message?: string; code?: string }; message?: string } } })?.response?.data
      setCodeError(tiktokErr?.tiktok_error?.message ?? tiktokErr?.tiktok_error?.code ?? tiktokErr?.message ?? 'เกิดข้อผิดพลาด')
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-2xl font-bold text-slate-800">ร้านค้า TikTok</h1>
          </div>
          <p className="text-slate-500 text-sm mt-1">
            ทั้งหมด {data?.total ?? 0} ร้านค้า · n8n จะ Sync เฉพาะร้านที่มี Access Token
          </p>
        </div>
        {can('tiktok-shops.create') && (
          <button
      ...
      <div className="relative max-w-sm">
        ...
        <input
          ...
          placeholder="ชื่อร้าน, Seller ID, Shops Code..."
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="text-left px-5 py-3 font-medium text-slate-600">Shops Code</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {isLoading ? (
              ...
              items.map((item) => {
                const tokenStatus = getTokenStatus(item)
                const expiresAt = item.access_token_expires_at ? new Date(item.access_token_expires_at) : null
                const daysLeft = expiresAt ? Math.ceil((expiresAt.getTime() - Date.now()) / 86400000) : null
                const expireLabel = expiresAt
                  ? expiresAt.toLocaleDateString('th-TH', { day: '2-digit', month: 'short', year: '2-digit' })
                  : null

                const refreshExpiresAt = item.refresh_token_expires_at ? new Date(item.refresh_token_expires_at) : null
                const refreshDaysLeft = refreshExpiresAt ? Math.ceil((refreshExpiresAt.getTime() - Date.now()) / 86400000) : null
                const refreshExpireLabel = refreshExpiresAt
                  ? refreshExpiresAt.toLocaleDateString('th-TH', { day: '2-digit', month: 'short', year: '2-digit' })
                  : null

                return (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-5 py-4">
                      <p className="font-semibold text-slate-800">{item.seller_name ?? '—'}</p>
                    </td>
                    <td className="px-5 py-4 font-mono text-xs text-slate-600">{item.seller_id || '—'}</td>
                    <td className="px-5 py-4">
                      {item.shops_code ? (
                        <span className="px-2 py-0.5 bg-rose-50 text-rose-700 rounded text-xs font-mono font-medium">
                          {item.shops_code}
                        </span>
                      ) : '—'}
                    </td>
                    ...
                    <td className="px-5 py-4 text-center">
                      {tokenStatus === 'none' ? (
                        can('tiktok-shops.edit') ? (
                          <button
                            onClick={() => handleGetAuthUrl(item)}
                            disabled={authUrlLoading === item.id}
                            className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-rose-500 hover:bg-rose-600 disabled:opacity-60 text-white rounded-full text-xs font-medium transition-colors"
                          >
                        ...
                        <div className="flex flex-col items-center gap-1">
                          ...
                          {expireLabel && (
                            <span className={`text-xs ${
                              tokenStatus === 'expired' ? 'text-red-500 font-medium' :
                              tokenStatus === 'critical' ? 'text-orange-500 font-medium' :
                              tokenStatus === 'warning' ? 'text-yellow-600' : 'text-slate-400'
                            }`}>
                              หมดอายุ {expireLabel}
                            </span>
                          )}
                          {can('tiktok-shops.edit') && (tokenStatus === 'warning' || tokenStatus === 'critical' || tokenStatus === 'expired') && (
                            <button
                    ...
                    <td className="px-5 py-4 text-center">
                      {!item.refresh_token ? (
                        ...
                        <div className="flex flex-col items-center gap-0.5">
                          ...
                          {refreshExpireLabel && (
                            <span className={`text-xs ${refreshDaysLeft != null && refreshDaysLeft <= 0 ? 'text-red-500' : 'text-slate-400'}`}>
                              หมดอายุ {refreshExpireLabel}
                            </span>
                          )}
                        </div>
                      )}
                    ...
                    <td className="px-5 py-4">
                      <div className="flex items-center justify-end gap-1">
                        ...
                        {can('tiktok-shops.delete') && (
                          <button
                            onClick={() => {
                              if (confirm(`ลบร้านค้า "${item.seller_name}"? ข้อมูลจะถูกลบถาวรและ n8n จะหยุด Sync ร้านนี้`)) {
      ...
      <p className="text-xs text-slate-400 flex items-center gap-1.5">
        ...
        กดปุ่ม &ldquo;ขอ Token&rdquo; เพื่อ Authorize กับ TikTok Shop · หน้า TikTok จะเปิดใน tab ใหม่ แล้วนำ code กลับมาวางที่นี่
      </p>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              ...
              <ol className="text-sm text-slate-500 space-y-1 list-decimal list-inside">
                <li>Authorize บน TikTok Shop ใน tab ที่เปิดใหม่</li>
      ...
      <div className="bg-white rounded-xl border border-slate-200">
        <div className="flex items-center gap-2 px-5 py-3 border-b border-slate-100">
          ...
          <h2 className="text-sm font-semibold text-slate-700">ประวัติการแก้ไขร้านค้า TikTok</h2>
          ...
          {auditLoading ? (
            ...
            auditData.data.map((log) => {
              const ACTION_LABEL: Record<string, { label: string; cls: string }> = {
                ...
                exchange_token: { label: 'ขอ Token ใหม่',  cls: 'bg-rose-100 text-rose-700' },
