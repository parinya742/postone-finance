...
export default function LineExtractedPage() {
  ...
  const { data, isLoading } = useQuery<PaginatedResponse<LineGroupExtractedFile>>({
    queryKey: ['line-extracted', search, typeFilter, page],
    queryFn: () =>
      api.get('/line-extracted', { params: { search, file_type: typeFilter, page, per_page: 20 } }).then((r) => r.data),
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-800">Extracted Header Files</h1>
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="text-left px-5 py-3 font-medium text-slate-600">ต้นทาง (Parent)</th>
              ...
              items.map((item) => (
                <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-5 py-4">
                    <p className="font-medium text-slate-800 truncate max-w-[200px]">{item.file_name ?? '—'}</p>
                    {item.file_extension && (
                      <span className="text-xs text-slate-400 font-mono">.{item.file_extension}</span>
                    )}
                  </td>
                  <td className="px-5 py-4">
                    {item.file_type ? (
                      <span className={clsx('px-2.5 py-1 rounded-full text-xs font-medium', TYPE_COLORS[item.file_type] ?? 'bg-slate-100 text-slate-600')}>
                        {item.file_type}
                      </span>
                    ) : '—'}
                  </td>
                  ...
                  <td className="px-5 py-4">
                    {item.s3_url && (
                      <a href={item.s3_url} target="_blank" rel="noopener noreferrer" className="p-1.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors inline-flex">
