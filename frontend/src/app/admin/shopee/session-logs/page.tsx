...
import { ShopeeSessionLog, PaginatedResponse } from '@/lib/types'
import { useQuery } from '@tanstack/react-query'
import { useState } from 'react'
import { Lock, History, CheckCircle2, XCircle, AlertTriangle, RefreshCw, Clock } from 'lucide-react'
...
export default function ShopeeSessionLogsPage() {
  const { can } = useAuth()
  ...
  const { data, isLoading, refetch, isFetching } = useQuery<PaginatedResponse<ShopeeSessionLog>>({
    queryKey: ['shopee-session-logs', sellerKey, status, triggeredBy, startDate, endDate, page],
    queryFn: () =>
      api.get('/shopee/session-logs', {
        params: {
  ...
  // track run_id to alternate row background per run group
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-slate-800">Cookie Capture Logs</h1>
            <p className="text-slate-500 text-sm mt-0.5">ประวัติการ Capture Cookie ทุกครั้ง</p>
          </div>
        </div>
        <button
          onClick={() => refetch()}
          ...
          <table className="w-full text-sm min-w-[850px]">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium text-slate-600">Missing / Reason</th>
              </tr>
            </thead>
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <p className="font-medium text-slate-700 text-xs">
                          {item.shop_name !== item.seller_key ? item.shop_name : null}
                        </p>
                      ...
                      <td className="px-4 py-3 text-xs text-slate-400 max-w-[220px]">
                        {item.missing && (
                          <span className="inline-block bg-yellow-50 text-yellow-700 border border-yellow-200 px-1.5 py-0.5 rounded font-mono mr-1 mb-0.5">
                        ...
                        {!item.missing && !item.reason && <span className="text-slate-300">—</span>}
                      </td>
                    </tr>
                  )
                })
