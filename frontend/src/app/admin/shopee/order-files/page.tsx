...
import { ShopeeOrderFile, ShopeeShop, PaginatedResponse } from '@/lib/types'
...
export default function ShopeeOrderFilesPage() {
  const { can } = useAuth()
  ...
  const { data, isLoading } = useQuery<PaginatedResponse<ShopeeOrderFile>>({
    queryKey: ['shopee-order-files', search, page, shopName],
    queryFn: () =>
      api.get('/shopee/order-files', {
    ...
    <div className="space-y-6">
      <div>
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-slate-800">ไฟล์ออเดอร์ Shopee</h1>
        </div>
                ...
                items.map((item) => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-5 py-4 text-xs font-mono text-slate-600 whitespace-nowrap">
                      {fmtDate(item.range_from)}
                      {item.range_from && item.range_to ? ' – ' : ''}
                      {fmtDate(item.range_to)}
                    </td>
